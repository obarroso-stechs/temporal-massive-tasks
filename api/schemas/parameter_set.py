...
class ParameterSetItem(BaseModel):
    serialNumber: str = Field(min_length=1)


class ParameterSetBatchRequest(BaseModel):
    items: List[ParameterSetItem] | None = None
    group_id: int | None = None
    parameters: dict[str, ParameterValue]

    @field_validator("parameters")
    @classmethod
    def validate_parameters_not_empty(cls, value: dict[str, ParameterValue]) -> dict[str, ParameterValue]:
        if not value:
            raise ValueError("parameters no puede estar vacio")
        return value

    @model_validator(mode="after")
    def validate_items_or_group(self) -> "ParameterSetBatchRequest":
...
class ParameterSetBatchScheduledRequest(ParameterSetBatchRequest):
...
class ParameterSetBatchStartResponse(BaseModel):
