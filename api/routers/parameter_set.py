...
router = APIRouter(prefix="/parameter-set", tags=["parameter-set"])


def get_workflow_status_service() -> WorkflowStatusService:
    return WorkflowStatusService(ParameterSetBatchWorkflow)


@router.post(
    "/batch/start-now",
    response_model=ParameterSetBatchStartResponse,
    status_code=status.HTTP_202_ACCEPTED,
)
async def start_batch_now(
    payload: ParameterSetBatchRequest,
    temporal_client: Client = Depends(get_temporal_client),
    orchestration: BatchOrchestrationService = Depends(get_batch_orchestration_service),
) -> ParameterSetBatchStartResponse:
    try:
        workflow_id, run_id = await orchestration.start_parameter_set_batch(
    ...
    return ParameterSetBatchStartResponse(
...
async def schedule_batch(
    payload: ParameterSetBatchScheduledRequest,
    temporal_client: Client = Depends(get_temporal_client),
    orchestration: BatchOrchestrationService = Depends(get_batch_orchestration_service),
) -> ParameterSetBatchStartResponse:
    try:
...
async def pause_batch(
    ...
    try:
        await handle.signal(ParameterSetBatchWorkflow.pause_batch)
...
async def resume_batch(
    ...
    handle = temporal_client.get_workflow_handle(workflow_id)
    try:
        await handle.signal(ParameterSetBatchWorkflow.resume_batch)
    except RPCError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc)) from exc
