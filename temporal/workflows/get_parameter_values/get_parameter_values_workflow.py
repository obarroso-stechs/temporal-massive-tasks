...
with workflow.unsafe.imports_passed_through():
    ...
    from temporal.workflows.get_parameter_values.get_parameter_values_child_workflow import (
...
@workflow.defn
class GetParameterValuesBatchWorkflow:
    """Workflow padre: lee parametros de miles de dispositivos en batches."""
    ...
    @workflow.run
    async def run(
        self, input: GetParameterValuesBatchInput
    ) -> List[Union[GetParameterValuesResult, str]]:
        ...
        results: List[Union[GetParameterValuesResult, str]] = []
    ...
    async def _execute_batch(
        self,
        batch: List[GetParameterValuesInput],
    ) -> List[Union[GetParameterValuesResult, str]]:
        parent_id = workflow.info().workflow_id

        async def run_one(
            idx: int,
            item: GetParameterValuesInput,
        ) -> tuple[int, Union[GetParameterValuesResult, BaseException]]:
            try:
                result = await workflow.execute_child_workflow(
                    GetParameterValuesChildWorkflow.run,
                    item,
        ...
        raw_results: list[Union[GetParameterValuesResult, str, None]] = [None] * len(batch)
